...
void NoiseSensor::update() {
  ...
  if (now - tmpIniMs > 1000) {
    ...
    if (shouldLog(LOG_VERBOSE)) {
      ESP_LOGD(TAG, "Noise:%lu loop:%d cycle:%lu loops_legal:%d",
               static_cast<unsigned long>(measurements.noise), loops,
...
void NoiseSensor::calculateLegalAverage() {
  ...
  if (shouldLog(LOG_DEBUG)) {
    ESP_LOGD(TAG, "(Legal) noise_avg_legal: %.2f noise_avg_legal_max: %.2f samples:%d",
             measurements.noiseAvgLegal, measurements.noiseAvgLegalMax, loopsLegal);
  }
}
